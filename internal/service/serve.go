...
// ServeSettings document (the server-instance list + the shared
// users/shares pools).
const metaServeSettings = "serve_settings"

// GetServeSettings returns the persisted file-serving configuration. A
// fresh install (no row yet) yields a zero-value ServeSettings with no
// servers configured, which is the safe default.
